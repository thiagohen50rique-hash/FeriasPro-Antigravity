        ...
        if (action === 'reject') {
            ...
            const signatureParticipant = createApproverSignatureParticipant(currentUser, request);
             const updatedSignatureInfo = {
                ...(request.signatureInfo as InformacaoAssinatura),
                participants: [...(request.signatureInfo?.participants || []), signatureParticipant],
            };
            updatedPeriod.signatureInfo = updatedSignatureInfo;

            if (request.status === 'pending_manager') {
                updatedPeriod.status = 'pending_rh';
                updatedPeriod.managerApproverId = currentUser.id;
                ...
                updatedPeriod.hrApproverId = currentUser.id;
