...
export const MOCK_HIERARCHY_LEVELS: NivelHierarquico[] = [
  { id: 1, nivel: 1, descricao: 'Operacional/Técnico' },
  { id: 2, nivel: 2, descricao: 'Coordenador' },
  { id: 3, nivel: 3, descricao: 'Supervisor' },
  { id: 4, nivel: 4, descricao: 'Gerente' },
  { id: 5, nivel: 5, descricao: 'Superintendente Executivo' },
  { id: 6, nivel: 6, descricao: 'Vice-Presidente' },
  { id: 7, nivel: 7, descricao: 'Presidente' },
];
...
export const MOCK_CONFIG: ConfiguracaoApp = {
  ...
  statusFerias: DEFAULT_VACATION_STATUSES,
};
...
export const getStatusText = (status: string, config?: ConfiguracaoApp) => {
  if (config && config.statusFerias) {
    const customStatus = config.statusFerias.find(s => s.id === status);
...
const getIsSuperiorInArea = (potentialSuperior: Funcionario, subordinate: Funcionario, orgUnits: UnidadeOrganizacional[]): boolean => {
  const subordinateArea = orgUnits.find(u => u.nome === subordinate.departamento && u.tipo === 'Área');
  if (!subordinateArea) return false;

  let currentAreaId = subordinateArea.idPai;
  while (currentAreaId) {
    const parentArea = orgUnits.find(u => u.id === currentAreaId);
    if (!parentArea) break;
    if (parentArea.nome === potentialSuperior.departamento) return true;
    currentAreaId = parentArea.idPai;
  }
  return false;
}
